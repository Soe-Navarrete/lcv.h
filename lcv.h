...
int Trip (float U1, float U2, float U3, float V1, float V2, float V3,float w1, float w2, float w3){
	...
	f2=(V1*w3)-(w1*V3);
	f3=(V1*w2)-(w1*V2);
	x1=(U1*f1)+(U2*f2)+(U3*f3);
	cout<<"("<<x1<<")"<<Trip<<endl;
}

int Devec (float x1, float x2, float x3, float y1, float y2, float y3, float z1, float z2, float z3){
	w=(x1*((y2*z3)-(z2*y3)))-(x2*((y1*z3)-(z1*y3)))+(x3*((y1*z2)-(z1*y2)));
	cout<<"("<<w<<")"<<Devec;
	return 0;
}

int Esca (float U1, float U2, float U3, float V1, float V2, float V3){
	x1=((U1*V1)+(U2*V2)+(U3*V3));
	x2=((U1*U1)+(U2*U2)+(U3*U3));
	x3=((V1*V1)+(V2*V2)+(V3*V3));
	w=(acos(x1/x2*x3));
	cout<<"("<<w<<")"<<Esca;
	return 0;
